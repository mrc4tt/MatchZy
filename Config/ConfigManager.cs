...
// Whether the demo upload uses HTTP PUT with the raw .dem file as the body, for S3-compatible
// storage that hands out presigned upload URLs. Default value: false (HTTP POST to a panel).
matchzy_demo_upload_s3 false

// Where recorded demos are uploaded once the map ends. Left unset on purpose: a presigned URL and
// an auth header are credentials, and this file is world-readable. Set them from a private cfg.
// A presigned S3 URL is also per-object and expires, so with matchzy_demo_upload_s3 true it has to
// be pushed per match over rcon rather than being a fixed value here.
// Always quote the URL. Syntax:
// matchzy_demo_upload_url ""https://your-panel.example/demo""
// matchzy_demo_upload_header_key ""Authorization""
// matchzy_demo_upload_header_value ""your-token""
