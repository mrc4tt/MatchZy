...
namespace MatchZy
{
    public partial class MatchZy
    {
        ...
        public FakeConVar<bool> matchSummaryPanelEnabled = new(
            "matchzy_match_summary_panel",
            "Show end-of-match center HTML panel with top fragger / clutch / rating. Default: true",
            true
        );

        public FakeConVar<int> matchSummaryPanelDuration = new(
            "matchzy_match_summary_panel_duration",
            "How many seconds to display the end-of-match summary panel. Default: 12",
            12
        );
        ...
        public FakeConVar<int> techPauseDuration = new(
            "matchzy_tech_pause_duration",
            "Tech pause duration in seconds. Default value: 300",
            300
        );

        public FakeConVar<int> maxTechPausesAllowed = new(
            "matchzy_max_tech_pauses_allowed",
            " Max tech pauses allowed. Default value: 2",
            2
        );

        public FakeConVar<bool> autoPauseEnabled = new(
            "matchzy_autopause_enabled",
            "Whether to automatically pause when a team has fewer than minimum players. Replaces sv_matchpause_auto_5v5. Default: true",
            true
        );

        public FakeConVar<int> autoPauseMinPlayers = new(
            "matchzy_autopause_minplayers",
            "Minimum players required per team before auto-pause triggers. Default: 5",
            5
        );

        public FakeConVar<int> autoResumeDelay = new(
            "matchzy_autopause_resume_delay",
            "Delay in seconds before auto-resuming when teams are balanced. Default: 3",
            3
        );

        public FakeConVar<string> teamNameCt = new(
            "matchzy_ct_name",
            "Set teamname for CT. Set to \"\" to disable/use default.",
            "CT"
        );

        public FakeConVar<string> teamNameT = new(
            "matchzy_t_name",
            "Set teamname for Terrorist. Set to \"\" to disable/use default.",
            "T"
        );
        ...
        public FakeConVar<bool> everyoneIsAdmin = new(
            "matchzy_everyone_is_admin",
            "If set to true, all the players will have admin privilege. Default: false",
            false
        );

        public FakeConVar<bool> allowPauseCommand = new(
            "matchzy_allow_pause",
            "Enable or disable .pause command",
            true
        );

        public FakeConVar<bool> allowUnpauseCommand = new(
            "matchzy_allow_unpause",
            "Enable or disable .unpause command",
            true
        );

        public FakeConVar<string> hostnameFormat = new(
            "matchzy_hostname_format",
            "The server hostname to use. Set to \"\" to disable/use existing. Default: MatchZy | {TEAM1} vs {TEAM2}",
            ""
        );
        ...
        public FakeConVar<bool> matchEndAutoChangelevel = new(
            "matchzy_match_end_auto_changelevel",
            "Whether to automatically change map after match end. Disable this for G5API/tournament matches. Default: true",
            true
        );
        ...
        public void MatchZySaveNadesAsGlobalConvar(CCSPlayerController? player, CommandInfo command)
        {
            ...
            isSaveNadesAsGlobalEnabled = bool.TryParse(
                args,
                out bool isSaveNadesAsGlobalEnabledValue
            )
        ...
        public void MatchZyResetCvarsOnSeriesEndConvar(
        ...
        [ConsoleCommand(
            ...
            "Minimum ready players required to start the match. Default: 10"
        )]
        ...
        [ConsoleCommand(
            ...
            "Whether .stop command is enabled or not (to restore the current round). Default value: true"
        )]
        ...
        public void MatchZyChatMessagesTimerDelay(CCSPlayerController? player, CommandInfo command)
        {
            ...
            if (command.ArgCount >= 2)
            {
                ...
                {
                    if (
                        int.TryParse(commandArg, out int chatTimerDelayValue)
                        && chatTimerDelayValue >= 0
                    )
                    {
                    ...
                    else
                    {
                        ...
                        ReplyToUserCommand(
                            player,
                            Localizer.ForPlayer(player, "matchzy.cvars.invalidvalue")
                        );
                    }
                }
            }
        ...
        public void MatchZyMaxSavedLastGrenadesConvar(
            CCSPlayerController? player,
            CommandInfo command
        )
        {
            ...
            else
            {
                ...
                ReplyToUserCommand(
                    ...
                    Localizer.ForPlayer(
                        player,
                        "matchzy.cc.usage",
                        $"matchzy_max_saved_last_grenades <number>"
                    )
                );
            }
        }
        ...
        [ConsoleCommand(
            ...
            "A URL to send backup files to over HTTP. Leave empty to disable."
        )]
        [ConsoleCommand(
        ...
        [ConsoleCommand(
            ...
            "If defined, a custom HTTP header with this name is added to the backup HTTP request."
        )]
        [ConsoleCommand(
        ...
        [ConsoleCommand(
            ...
            "If defined, the value of the custom header added to the backup HTTP request."
        )]
        [ConsoleCommand(
